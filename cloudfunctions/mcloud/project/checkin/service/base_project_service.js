...
class BaseProjectService extends BaseService {
	...
	async initSetup() {
		let F = (c) => 'bx_' + c;
		const INSTALL_CL = 'setup_checkin';
		const COLLECTIONS = ['setup', 'admin', 'log', 'news', 'enroll', 'enroll_join', 'enroll_user', 'fav', 'user'];
		const CONST_PIC = '/images/cover.gif'; 

		const NEWS_CATE = '1=通知公告';
		const ENROLL_CATE = '1=学习,2=兴趣,3=生活,4=运动,5=工作';
